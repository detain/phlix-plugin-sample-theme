...
/**
 * S440 (finishing S246) — give this repository a security audit gate that covers
 * the WHOLE lock, states the corpus it examined, and is capable of failing.
 *
 * ## The defect this closes, measured on this repo at 1ea573b (2026-09-05)
 *
 * This repository had **no security audit gate at all** — no lock, no script, no
 * CI leg. It is the last of the eleven plugin repositories left ungated. Nothing
 * in CI ever asked whether a locked dependency carried a published advisory, so
 * the answer was never wrong — it was never given. The earlier S246 sweep recorded
 * this repo as gated using TRAKT's statistics (a wrong-directory measurement);
 * the repo itself was never checked on disk until S440. This script is the parity
 * that audit32 found missing.
 *
 ...
 *
 * Declared as a constant so {@see \Phlix\PluginSampleTheme\Tests\SecurityAuditCheckTest} can assert the list directly
 ...
 *
 * Measured on this repository's committed lock at 1ea573b (2026-09-05): 33
 * packages (5 require + 28 require-dev). The floor sits below that so ordinary
