 ...
 *
 * Lane token: S440SAMPTHEMEGATEX6B2.
 *
 * ## The defect these tests pin shut
 *
 * This repository — the reference ui-theme plugin — had **no security audit gate
 * at all**: no committed lock, no gate script, no CI leg. The S246 sweep recorded
 * it as gated using TRAKT's statistics (a wrong-directory measurement); the repo
 * itself was never audited on disk until S440. A green that cannot go red is not
 * evidence.
 *
...
final class SecurityAuditCheckTest extends TestCase
{
    ...
    /** S440 lane survival token — asserted against the gate script by {@see testTheLaneTokenIsCarriedByTheGateScript()}. */
    private const LANE_TOKEN = 'S440SAMPTHEMEGATEX6B2';

    /**
     * Floors the script enforces, restated here deliberately.
     *
     * Measured on this repository's committed lock at 1ea573b (2026-09-05):
    ...
    /**
     * S440 lane marker: the gate suite belongs to lane `s440`, and the token is
     * real code (a string constant) so it survives tokenisation rather than
     * living only in a comment that php_strip_whitespace would erase.
     */
    public function testThisIsTheS440GateSuite(): void
    {
        self::assertSame('S440SAMPTHEMEGATEX6B2', self::LANE_TOKEN);
    }
    ...
    /**
     * The exact 2026-08-06 finding, replayed against THIS repo's real committed
     * lock (which carries squizlabs/php_codesniffer 4.0.4 in require-dev). Plant
     * an advisory that names it at an affected version and the gate must go RED
     * with the CVE verbatim; remove the advisory and the same lock goes GREEN.
     */
